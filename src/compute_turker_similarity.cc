...
float editdistance(const char* a, const char* b) {
	...
	for(int i = 1; i <= len1; i++) {
		for(int j = 1; j <= len2; j++) {
			float cost = (wrds1[i-1] == wrds2[j-1]) ? 0 : scost;
			float mn = std::min(dist_matrix[i-1][j]+idcost,dist_matrix[i][j-1]+idcost); 
			dist_matrix[i][j] = (mn < dist_matrix[i-1][j-1]+cost) ? mn : dist_matrix[i-1][j-1]+cost; 
		}
	}
	float edist = dist_matrix[len1][len2];
	for(int i = 0; i <= len1; i++)
...
// Function that determines how to convert the edit distance into 
// a score that increases with better matches.
...
int main(int argc, char** argv) {
	...
	string line;
	vector<pair<float,int> > turkscores;
	...
	while(!ifile.eof()) {
		...
		if(!line.empty()) {
			...
			double bestscore = turkscores[0].first;
			double worstscore = turkscores[turkscores.size()-1].first;
			assert (bestscore < 0);
            float simscore = 1.0;
