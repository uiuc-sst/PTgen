#!/usr/bin/perl
#
# Convert Carmel-style FST to an OpenFST-style FST
...
while(<STDIN>) {
	...
	if($#fields > 1) { #arc
		...
		if($start_flag == 1) {
			...
			$start_flag = 0;
		} elsif(!exists $state_names{$start_state}) { #start state is the first field
